    ...
    for index in range(len(start_time)):
        ...
        if end_ind - str_ind > seg_len:
            ...
            while end_ind - (str_ind + shift) > seg_len:
                ...
                if np.mean(hf['features'][file_index]) == 0:
                    print("Warning!!!!")
...
def feature_transform(conf=None,mode=None):
    ...
    if mode == 'train':
        ...
        for file in all_csv_files:
            ...
            print("Index : {}    Processing file name {}".format(index,audio_path))
            h5_path = os.path.basename(file).replace('csv', 'h5')

            hdf_tr = os.path.join(conf.path.feat_train, h5_path)
            ...
            hf.create_dataset('labels', data=[s.encode() for s in label_list], dtype='S20')
