...
class Datagen(object):
    ...
    def __init__(self, conf):
        all_h5_file = os.listdir(conf.path.feat_train)
        self.labels = []
        self.x = []
        for file in all_h5_file:
            hdf_path = os.path.join(conf.path.feat_train, file)
            hdf_train = h5py.File(hdf_path, 'r')
            x = hdf_train['features'][:] # [21489,17,128]
            labels = [s.decode() for s in hdf_train['labels'][:]] # ["str"] length 21489
            hdf_train.close()
            for i in range(len(x)):
                if np.mean(x[i]) == 0:
                    print("Remove Index: {} dataset: {} label:{}".format(i,file,labels[i]))
                else:
                    self.x.append(x[i])
        ...
        self.y = class_to_int(self.labels,class_set) # convert string label to int
        self.x,self.y = balance_class_distribution(self.x,self.y) # [188058,17,128]

        array_train = np.arange(len(self.x))
        _,_,_,_,train_array,valid_array = train_test_split(self.x,self.y,array_train,random_state=42,stratify=self.y, test_size=0.1)
